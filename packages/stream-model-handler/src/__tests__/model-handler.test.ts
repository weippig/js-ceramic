...
describe('ModelHandler', () => {
  ...
  // it('Anchor Version 1 works correctly', async () => {
  //   // const genesisCommit = (await Model._makeGenesis(
  //   //   context.api,
  //   //   PLACEHOLDER_CONTENT
  //   // )) as SignedCommitContainer
  ...
  //could merge into anchor 0 test
  it('applies version 1 anchor commit correctly', async () => {
    /* 
    to get around:
    invalid_jws: signature authored with a 
    revoked DID 
    version: did:3:k2t6wyfsu4pg0t2n4j8ms3s33xsgqjhtto04mvq8w5a2v5xo48idyz38l7ydki?version=0#signing
    */
    const rotateDate = new Date('2022-03-11T21:28:07.383Z'); //new date 
    rotateKey(did, rotateDate.toISOString())
    const genesisCommit = (await Model._makeGenesis(
      context.api,
      PLACEHOLDER_CONTENT
